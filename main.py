...
class LabirintTurtle:
    ...
    def find_exit(self):
        if self.turtle_map:
            x = int(self.x_start)
            y = int(self.y_start)
            n = len(self.turtle_map)
            m = len(self.turtle_map[0])
            copy_turtle_map = copy.deepcopy(self.turtle_map)
            copy_turtle_map = search(x, y, 0, n, m, copy_turtle_map)
    ...
    def exit_show_step(self):
        if self.exits:
            current_coords = min_coords(self.exits)
